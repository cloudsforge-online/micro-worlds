 ...
 * (`parseServiceGrants`, identity/src/env.ts). An unregistered demand here is a dead identity
 ...
 * (market/src/ledgerclient.ts). The ledger's account key is `(subject, asset_code, purpose)`
 * and `ensureAccount` THROWS `AccountConflictError` when a caller's type disagrees with the
 * existing row (ledger/src/accounts.ts) — so whichever service posted second would have had
